...
create_network() {
    "$DOCKER" network inspect "$NETWORK_DOMAIN" >/dev/null 2>&1 &&
        echo "Network '$NETWORK_DOMAIN' already exists, skipping creation." ||
        { "$DOCKER" network create "$NETWORK_DOMAIN" >/dev/null && echo "Network '$NETWORK_DOMAIN' created successfully."; }
...
create_forgejo_container() {
    ...
    "$DOCKER" exec -u $MYCLAW_USER "$FORGEJO_CONTAINER_NAME" grep -Eq '^\s*INSTALL_LOCK\s*=\s*true' "$forgejo_config" >/dev/null 2>&1 &&
...
create_git_user() {
    local git_user_name="$1" git_repo_name="$2" dev_home_dir="$3"

    "$DOCKER" exec -u $MYCLAW_USER "$FORGEJO_CONTAINER_NAME" bash -c "
        set -euo pipefail
        . forgejo_admin_token
        curl -4fs 'http://localhost:$FORGEJO_PORT/api/v1/users/$git_user_name' -H \"Authorization: token \$FORGEJO_ADMIN_TOKEN\" >/dev/null
        " && echo "Git user '$git_user_name' already exists in Forgejo, skipping creation." || {
            echo "Creating git user '$git_user_name' in Forgejo..."

            local git_user_password=$("$DOCKER" exec -u $MYCLAW_USER "$FORGEJO_CONTAINER_NAME" bash -c "
                set -euo pipefail
                . forgejo_admin_token
                password=\"\$(pwgen -cnsB 20 1)\"
...
grant_git_repository_write() {
    ...
    "$DOCKER" exec -u $MYCLAW_USER "$FORGEJO_CONTAINER_NAME" bash -c "
        set -euo pipefail
        . forgejo_admin_token
        curl -4fs 'http://localhost:$FORGEJO_PORT/api/v1/repos/$MYCLAW_ORG/$git_repo_name/collaborators/$git_user_name' -H \"Authorization: token \$FORGEJO_ADMIN_TOKEN\" >/dev/null &&
...
create_bifrost_container() {
    ...
    "$DOCKER" exec -u $MYCLAW_USER "$BIFROST_CONTAINER_NAME" test -f bifrost_admin_credential >/dev/null 2>&1 &&
...
save_bifrost_virtual_key() {
    ...
    [ -f "$path" ] || {
        ...
        local api_key=$(create_bifrost_virtual_key "$name")
...
initialize_home_dir() {
    ...
    mkdir -p "$home_dir"
    rsync -ar --ignore-existing "$CONF_DIR/home/$MYCLAW_USER/" "$home_dir"
}
